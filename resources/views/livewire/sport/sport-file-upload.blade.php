<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500">
        ...
        <a href="/sports/{{ $sport->id }}/file" class="font-bold text-black border-b-2 border-b-green-600">Upload
            File</a>
    </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-col mx-4 my-2 pt-4 pb-1 border-b-2 border-b-green-600 text-xl font-semibold">
            <h2>Sport <span class="text-sm"> (ID: {{ $sport->id }})</span></h2>
            <span class="text-gray-600 text-sm italic font-thin">{{ $sport->title }}</span>
        </div>

        <!-- Files Associated to the Sport -->
        <div class="mx-4 py-1 text-xs font-semibold">Files in this sport ({{ $sport->files->count() }} of 5)</div>
        ...
        @if ($sport->files->count() >= 5)
            <div class="mx-4 py-4 text-lg text-red-400 font-semibold">You have reached the limit of files for this
                sport. Delete some to upload new ones.</div>
            <div class="mx-4 py-8">
                <!-- Back -->
        ...
        @else
            <div class="mx-4 py-4">
                <form wire:submit.prevent="save">
                    ...
                    @if (count($files) + $sport->files->count() > 5)
                        <div class="my-4">
                            <span class="text-red-600 font-semibold">You have reached the limit of files
                                ({{ count($files) + $sport->files->count() }}) for this sport. Delete some to
                                upload
                                new ones.</span>
                        </div>
                    @else
